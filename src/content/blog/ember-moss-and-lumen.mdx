...
<MediaFigure
  src="/blog/ember-moss-lumen/observatory-fullplayer.gif"
  posterSrc="/blog/ember-moss-lumen/observatory-fullplayer-poster.png"
  alt="The replay player: three pixel-art agents clearing a room while their shared plan and messages update below"
  width={1080}
  height={923}
  caption="A flash-forward to where the day ends up – the observatory, the hardest room the agents cleared, replayed with the team's committed plan and messages alongside. The rest of this post is how we got here."
>
  <Fragment slot="description">
    An animated recording of the full replay player. In the world view, Ember
    and Lumen hold the Tide and Crown controls to keep the two-person Zenith
    lock open while Moss carries the cobalt lens through the exit; beneath the
    world, the shared plan panel shows the committed plan and the team bus shows
    the agents' actual negotiation messages updating as the run progresses,
    ending with all three agents at the exit and the objective complete.
  </Fragment>
</MediaFigure>
...
{/* IMAGE SLOT: Sam is making a picture of the room to open this section. */}
...
{/* DIAGRAM SLOT (hero): stepped walkthrough of the final gate, drawn from the trace. Steps roughly: (1) Flint + Quill held, Barrier III open; (2) both holders ordered toward the exit; (3) controls released, door closed; (4) opening restored, experiment cited; (5) same walk proposed in smaller increments; (6) closed again. Real positions/step data to come from the exported relay-events traces. */}
...
{/* DIAGRAM SLOT (second): the memory stack as destination – journal → episodic → semantic → working → procedural, each layer annotated with the observed failure that forced it. Conceptual, not architectural. */}
